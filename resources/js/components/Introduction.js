...
export function Introduction(props) {
    ...
    return (
        <section id="introduction" ref={section} className={" relative flex justify-center flex-col items-start"+(props.siteEntered ? ' moved-in' : ' opacity-0')}>
            <h1 className="w-full  text-4xl md:text-6xl font-bold opacity-0 details-color">
                Hi,
            </h1>

            <h2  className="w-full text-4xl md:text-6xl font-bold opacity-0 whitespace-nowrap details-color">
                I am Yannis.
            </h2>

            <h3 className="heading mt-4 md:mt-8 font-bold flex h-fit opacity-0">
                I build stuff for the web.
            </h3>
            <p className="opacity-0 mt-4 md:mt-8">
            ...
            <button className="text-2xl flex rounded-full opacity-0 mt-4 md:mt-8 p-5 my-pink-bg">
                Contact Me
            </button>
        </section>
    );
